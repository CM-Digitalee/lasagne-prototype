...
export class BudgetVisualizationComponent {
  ...
  simulationMode$ = this.route.queryParams.pipe(
    pluck('simulation'),
    map(state => state !== undefined ? true : false),
  ...
  openDistributionDialog(post, validatedMonth: number, distributionForm: DistributionFormDirective) {
    this.dialog.open(DistributionComponent, { width: '1000px', data: { post, validatedMonth, distributionForm } });
  }
  ...
  toggleSimulation(state: boolean) {
    this.router.navigate(['./'], { relativeTo: this.route, queryParams: state && { simulation: '' } });
  }
